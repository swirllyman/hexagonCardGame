...
export const ActionQueue: React.FC<ActionQueueProps> = ({
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between bg-slate-950/75 border border-amber-600/20 rounded-none p-2.5 shadow-inner">
          ...
          {programmedQueue.map((card, slotIdx) => {
            ...
            return (
              <div
                ...
                className={`relative ${slotHeightClass} rounded-none border-2 flex flex-col items-center justify-center p-1 text-center cursor-pointer transition-all duration-300 ${
              ...
              >
                ...
                {/* Slot Number Badge */}
                <div className="absolute -top-2 left-1.5 bg-slate-950 border border-slate-800 text-[9px] font-mono font-bold text-slate-300 px-1 py-0.2 rounded-none z-20">
                  S{slotIdx + 1}
                </div>
                  ...
                  <div className="flex flex-col items-center justify-center w-full relative h-full gap-1 py-0.5">
                    {!isLocked && (
                      <button
                        ...
                        className="absolute -top-1.5 -right-1.5 text-slate-400 hover:text-rose-400 bg-slate-950 rounded-none p-0.5 border border-slate-800 shadow z-20"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    )}
                    <div className="flex items-center justify-center bg-slate-900/80 border border-amber-500/40 rounded-none p-1 shadow-inner">
                      <SafeImage
                        src={card.spriteUrl}
                        alt={card.name}
                    ...
                    <span className="text-[10.5px] font-bold text-slate-100 line-clamp-1 leading-tight tracking-tight">{card.name}</span>
                  </div>
                ) : (
                  <div className="text-slate-500 flex flex-col items-center justify-center gap-0.5 h-full opacity-70 hover:opacity-100 transition-opacity">
                    <Sparkles className="w-3.5 h-3.5 text-slate-600 animate-pulse" />
                    <span className="text-[9px] font-mono font-bold tracking-tight uppercase">Empty</span>
                  </div>
                )}
              </div>
            );
      ...
      {/* Sharp Rectangular Emerald FIGHT Button with Metallic Gold End-Caps */}
      <button
        onClick={onLockIn}
        disabled={isLocked}
        className={`w-full py-2.5 mt-2.5 sharp-fight-btn font-mono font-black uppercase tracking-widest text-sm flex items-center justify-center gap-2 transition-all duration-300 shadow-2xl ${
          isLocked
            ? 'opacity-50 cursor-not-allowed'
            : 'text-teal-100 hover:scale-[1.02] cursor-pointer'
        }`}
      >
        {isLocked ? (
          <>
